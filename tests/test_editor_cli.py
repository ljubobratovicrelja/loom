...
class TestEditorCLI:
    ...
    def test_config_not_found_returns_error(self, tmp_path: Path, capsys) -> None:
        ...
        from loom.editor.cli import main

        nonexistent = tmp_path / "missing.yml"

        with patch("sys.argv", ["loom-editor", str(nonexistent)]):
            result = main()
    ...
    def test_new_flag_allows_missing_config(self, tmp_path: Path) -> None:
        ...
        with patch("sys.argv", ["loom-editor", str(nonexistent), "--new"]), \
             patch("loom.editor.cli.webbrowser.open"), \
             patch("loom.editor.cli.uvicorn.run") as mock_uvicorn:
            result = main()
    ...
    def test_tasks_dir_defaults_to_config_parent(self, tmp_path: Path) -> None:
        ...
        with patch("sys.argv", ["loom-editor", str(config)]), \
             patch("loom.editor.cli.webbrowser.open"), \
             patch("loom.editor.cli.uvicorn.run"), \
             patch("loom.editor.server.configure") as mock_configure:
            main()
    ...
    def test_tasks_dir_override(self, tmp_path: Path) -> None:
        ...
        with patch("sys.argv", ["loom-editor", str(config), "--tasks-dir", str(custom_tasks)]), \
             patch("loom.editor.cli.webbrowser.open"), \
             patch("loom.editor.cli.uvicorn.run"), \
             patch("loom.editor.server.configure") as mock_configure:
            main()
    ...
    def test_tasks_dir_fallback_for_new_without_config(self) -> None:
        ...
        with patch("sys.argv", ["loom-editor", "--new"]), \
             patch("loom.editor.cli.webbrowser.open"), \
             patch("loom.editor.cli.uvicorn.run"), \
             patch("loom.editor.server.configure") as mock_configure:
            main()
    ...
    def test_no_browser_flag_skips_browser_open(self, tmp_path: Path, capsys) -> None:
        ...
        with patch("sys.argv", ["loom-editor", str(config), "--no-browser"]), \
             patch("loom.editor.cli.webbrowser.open") as mock_browser, \
             patch("loom.editor.cli.uvicorn.run"):
            main()
