...
class TestGetCleanablePaths:
    ...
    def test_returns_all_data_paths(self, config: PipelineConfig) -> None:
        """Should return paths for all data nodes."""
        paths = get_cleanable_paths(config)

        # Get just the names (first element of each tuple)
        names = {name for name, _, _ in paths}

        # Should include all data nodes
...
class TestCleanPipelineData:
    ...
    def test_trash_mode_uses_send2trash(
        ...
        # send2trash should have been called for existing files
        assert mock_trash.call_count == 2  # input.txt and output.csv

    def test_permanent_mode_deletes_files(
        self, config: PipelineConfig, sample_pipeline_dir: Path
    ) -> None:
        """Should permanently delete files when permanent=True."""
        ...
        # Results should reflect successful deletion
    ...
    def test_skips_nonexistent_files(self, config: PipelineConfig) -> None:
        ...
        # Find result for 'missing' data node
        skipped = [r for r in results if r.action == "skipped"]

        # Should have skipped at least one (missing.txt and .loom-thumbnails)
    ...
    def test_reports_errors_on_failure(
        ...
        # Should have failed for existing files
        assert len(failed) == 2
