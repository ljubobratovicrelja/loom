...
const TYPE_COLORS: Record<DataType, string> = {
  video: 'bg-rose-900/50 text-rose-300 border-rose-600',
  image: 'bg-amber-900/50 text-amber-300 border-amber-600',
  csv: 'bg-emerald-900/50 text-emerald-300 border-emerald-600',
  json: 'bg-sky-900/50 text-sky-300 border-sky-600',
  image_directory: 'bg-orange-900/50 text-orange-300 border-orange-600',
  data_folder: 'bg-teal-900/50 text-teal-300 border-teal-600',
}
...
const getTypeInfo = (type: DataType) => {
  ...
  return {
    ...
    colors: TYPE_COLORS[type] || 'bg-slate-700 text-slate-300 border-slate-600',
  }
}
...
export default function PropertiesPanel({
  ...
  if (!selectedNode) {
    return (
      <div className="flex-1 bg-slate-900 border-l border-slate-700 p-4">
  ...
  return (
    <div className="flex-1 bg-slate-900 border-l border-slate-700 flex flex-col">
      ...
      <div className="p-4 space-y-4 overflow-y-auto flex-1">
            ...
            {onTrashData && (editData.exists as boolean) && (
              <div className="pt-2">
                <button
                  ...
                  className="w-full px-3 py-2 bg-orange-700 hover:bg-orange-600 text-white text-sm rounded transition-colors flex items-center justify-center gap-2"
                >
        ...
        {isParameter && (
          <>
            <div>
              <label className="block text-slate-400 text-xs mb-1">Name</label>
              <div className="px-3 py-2 bg-slate-800 border border-slate-700 rounded text-purple-300 text-sm">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded text-white text-sm focus:border-purple-500"
              />
            </div>
          ...
          <>
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded text-white text-sm focus:border-teal-500"
              />
            </div>
            <div>
              <label className="block text-slate-400 text-xs mb-1">Key (for $references)</label>
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-slate-800 border border-slate-700 rounded text-white text-sm font-mono focus:border-teal-500"
                />
              </div>
              <p className="text-slate-500 text-xs mt-1">
            ...
            <div>
              <label className="block text-slate-400 text-xs mb-1">Description (optional)</label>
              <input
                type="text"
            ...
            {isDirectoryType((editData.type as DataType) || 'data_folder') && (
              <div>
                <label className="block text-slate-400 text-xs mb-1">Pattern (optional)</label>
                <input
                  type="text"
            ...
            {editData.exists !== undefined && (
              <div className="flex items-center gap-2 text-xs">
                {editData.exists ? (
                  <span className="text-teal-400">&#10003; Path exists</span>
                ) : (
                  <span className="text-slate-500">&#9675; Path not found</span>
                )}
              </div>
            )}
            {/* Trash data button */}
        ...
        {isStep && (
          <>
            ...
            <div>
              <label className="block text-slate-400 text-xs mb-1">Task</label>
              <input
                type="text"
            ...
            <div>
                    ...
                    <div className="flex gap-2 mb-1">
                      <div className="w-1/3 flex items-center gap-1">
                        <input
                          ...
                          value={key}
                          disabled
                          className="flex-1 px-2 py-1 bg-slate-700 border border-slate-600 rounded text-slate-400 text-xs"
                        />
                      </div>
                      <input
                        type="text"
              ...
              {Object.keys((editData.inputs as Record<string, string>) || {}).length === 0 && (
                <p className="text-slate-600 text-xs italic">No inputs defined</p>
              )}
            </div>

            {/* Outputs */}
            <div>
              <label className="block text-slate-400 text-xs mb-2">Outputs</label>
                  ...
                  <div key={key} className="mb-3">
                    <div className="flex gap-2 mb-1">
                      <input
                        ...
                        value={key.replace(/^-+/, '')}
                        disabled
                        className="w-1/3 px-2 py-1 bg-slate-700 border border-slate-600 rounded text-slate-400 text-xs"
                      />
                      <input
                        type="text"
                        value={value}
                        disabled
                        className="flex-1 px-2 py-1 bg-slate-800 border border-slate-700 rounded text-slate-400 text-xs"
              ...
              {Object.keys((editData.outputs as Record<string, string>) || {}).length === 0 && (
                <p className="text-slate-600 text-xs italic">No outputs defined</p>
              )}
            </div>

            {/* Args */}
            <div>
              <label className="block text-slate-400 text-xs mb-2">Arguments</label>
                ...
                return (
                  <div key={key} className="mb-3">
                      ...
                      {isConnected ? (
                        ...
                        <>
                          <input
                            ...
                            value={key.replace(/^-+/, '')}
                            disabled
                            className="w-1/2 px-2 py-1 bg-slate-700 border border-slate-600 rounded text-slate-400 text-xs"
                          />
                          <div
                            className="flex-1 px-2 py-1 bg-purple-900/30 border border-purple-600 rounded text-purple-300 text-xs cursor-pointer flex items-center justify-between"
                            onContextMenu={(e) => {
                              e.preventDefault()
                          ...
                          {canRemove && (
                            <button
                              ...
                              className="px-2 py-1 bg-slate-700 hover:bg-red-700 text-slate-400 hover:text-white text-xs rounded transition-colors"
                        ...
                        <>
                          <div className="flex-1 px-2 py-1 bg-green-900/30 border border-green-700 rounded text-green-400 text-xs flex items-center gap-2">
                            <span>&#10003;</span>
                        ...
                        <>
                          ...
                          <input
                            ...
                            className={`flex-1 px-2 py-1 bg-slate-800 border rounded text-white text-xs ${
                              isDefault ? 'border-slate-700' : 'border-blue-600'
                            }`}
                          />
                          {canRemove && (
                            <button
                    ...
                    {argSchema && !isBoolFlag && !isConnected && (
                      <div className="text-xs ml-1">
                        {argSchema.description && (
                          <div className="text-slate-500 mb-0.5">{argSchema.description}</div>
                        )}
                    ...
                    {argSchema && isBoolFlag && argSchema.description && !isConnected && (
                      <div className="text-xs ml-1 text-slate-500">{argSchema.description}</div>
                    )}
                  </div>
                )
              })}
              {Object.keys((editData.args as Record<string, unknown>) || {}).length === 0 && (
                <p className="text-slate-600 text-xs italic">No arguments configured</p>
              )}
            </div>
            ...
            {availableArgs.length > 0 && (
              <div className="border-t border-slate-700 pt-3 mt-3">
                <label className="block text-slate-400 text-xs mb-2">Available Arguments</label>
                <div className="space-y-2">
                  {availableArgs.map(([key, schema]) => (
                    <button
                      key={key}
                      onClick={() => handleAddArg(key)}
                      className="w-full text-left px-2 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-blue-600 rounded text-xs transition-colors group"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-blue-400 font-mono">{key.replace(/^-+/, '')}</span>
                          {schema.type === 'bool' && (
                            <span className="text-slate-500 text-xs">(flag)</span>
                          )}
                        </div>
                        <span className="text-slate-500 group-hover:text-blue-400">+ Add</span>
                      </div>
                      {schema.description && (
                        <div className="text-slate-500 text-xs mt-1 truncate">{schema.description}</div>
                      )}
                      {schema.type !== 'bool' && (
                        <div className="flex gap-2 text-slate-600 text-xs mt-1">
            ...
            {onRunStep && onCancelStep && stepName && (
              <div className="border-t border-slate-700 pt-4 mt-4">
                <div className="flex items-center justify-between mb-2">
                  <label className="text-slate-400 text-xs">Execution</label>
                  {/* Freshness badge */}
                  {freshness && (
                    <span
                ...
                {stepStatus === 'running' ? (
                  <button
                    ...
                    className="w-full px-3 py-2 bg-red-700 hover:bg-red-600 text-white text-sm rounded transition-colors flex items-center justify-center gap-2"
                  >
                ...
                {stepStatus === 'completed' && (
                  <p className="text-green-400 text-xs mt-1 text-center">&#10003; Completed</p>
                )}
                {stepStatus === 'failed' && (
                  <p className="text-red-400 text-xs mt-1 text-center">&#10007; Failed</p>
                )}
              </div>
            )}
            ...
            <div className="border-t border-slate-700 pt-4 mt-4">
              <button
                ...
                className="flex items-center gap-2 text-slate-400 text-xs mb-2 hover:text-white"
              >
                      ...
                      <div className="space-y-1">
                        {Object.entries(parameters).map(([name, value]) => (
                          <div
                            ...
                            className="flex items-center justify-between px-2 py-1 bg-purple-900/30 hover:bg-purple-800/40 rounded cursor-pointer"
                            title="Click to copy"
                          >
