...
from loom.runner.multi_pass import expand_multi_pass
...
def _flatten_pipeline(
    ...
    Multi-pass groups are expanded into concrete per-pass steps.
    Ungrouped steps are passed through unchanged.

    Returns:
        Tuple of (flat_steps, multi_pass_groups_dict, extra_data_entries).
        ``multi_pass_groups_dict`` maps group names to their original YAML blocks
        for round-trip preservation. ``extra_data_entries`` contains suffixed data
        entries generated by multi_pass expansion.
    """
    ...
    for entry in pipeline:
        if "group" in entry and "steps" in entry:
            if "multi_pass" in entry:
                # Multi-pass group: expand and store original for round-trip
                expansion = expand_multi_pass(
                ...
                # Create data entries for suffixed variables
                for var_name, var_path in expansion.extra_variables.items():
                    dtype = expansion.extra_data_types.get(var_name, "")
                    extra_data[var_name] = {"type": dtype, "path": var_path}
            else:
    ...
    return flat, multi_pass_groups, extra_data
...
def _collect_multi_pass_data_keys(multi_pass_groups: dict[str, Any]) -> set[str]:
    """Collect the set of suffixed data keys generated by multi_pass expansions.

    These keys should be excluded when writing back to YAML since they are
    auto-generated from the template during expansion.
    """
    generated: set[str] = set()
    for _group_name, mp_info in multi_pass_groups.items():
        mp_data = mp_info.get("multi_pass", {})
        template_steps = mp_info.get("steps", [])
        passes = mp_data.get("passes", [])

        # Collect internal output variable names from template steps
...
def yaml_to_graph(data: dict[str, Any]) -> PipelineGraph:
    ...
    all_steps, multi_pass_groups, extra_data = _flatten_pipeline(pipeline, data_section)

    # Merge suffixed data entries from multi_pass expansion into data_section
    # (so data nodes and edges are created automatically)
    data_section = dict(data_section)  # copy to avoid mutating original
    data_section.update(extra_data)
...
def graph_to_yaml(graph: PipelineGraph) -> dict[str, Any]:
    ...
    for node in step_nodes:
        ...
        # Skip unrolled multi_pass steps — emit original block instead
    ...
    # Collect generated data keys from multi_pass expansions (to exclude from YAML)
    mp_generated_keys = _collect_multi_pass_data_keys(graph.multiPassGroups)
    ...
    for node in graph.nodes:
        if node.type == "data":
            ...
            # Skip suffixed data nodes generated by multi_pass expansion
            if data_key in mp_generated_keys:
                continue
            entry: dict[str, Any] = {
                "type": node.data["type"],
                "path": node.data["path"],
    ...
    for name, data_entry in graph.data.items():
        if name not in data_section and name not in mp_generated_keys:
...
def update_yaml_from_graph(data: dict[str, Any], graph: PipelineGraph) -> None:
    ...
    # Collect unrolled step names from multi_pass groups (skip during updates)
    mp_unrolled_names: set[str] = set()
    for node in step_nodes:
        group = node.data.get("group")
        if group and group in graph.multiPassGroups:
            mp_unrolled_names.add(node.data["name"])
    ...
    for name, graph_step in graph_steps.items():
        # Skip unrolled multi_pass steps — they are auto-generated
        if name in mp_unrolled_names:
            continue
